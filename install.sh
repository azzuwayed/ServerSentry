...
# Print header
echo -e "${GREEN}ServerSentry - System Monitoring & Alert Tool Installer${NC}"
echo "==========================================================="
echo ""
...
# Setup cron job
echo ""
echo "Do you want to set up a cron job to run ServerSentry periodically?"
read -p "Setup cron job? (y/n): " setup_cron

if [[ "$setup_cron" =~ ^[Yy]$ ]]; then
    CRON_INTERVAL="*/5"
    echo ""
    echo "How often should ServerSentry run?"
    ...
    if [ -n "$custom_cron" ]; then
        CRON_ENTRY="$custom_cron $SCRIPT_DIR/serversentry.sh --check >> $SCRIPT_DIR/serversentry.log 2>&1"
    else
        CRON_ENTRY="$CRON_INTERVAL * * * * $SCRIPT_DIR/serversentry.sh --check >> $SCRIPT_DIR/serversentry.log 2>&1"
    fi
    
    # Add to crontab
    (crontab -l 2>/dev/null || echo "") | grep -v "$SCRIPT_DIR/serversentry.sh" | { cat; echo "$CRON_ENTRY"; } | crontab -
    
    echo -e "${GREEN}Cron job has been set up successfully!${NC}"
    echo "ServerSentry will run according to the schedule you specified."
else
...
echo "  $SCRIPT_DIR/serversentry.sh --add-webhook https://your-webhook-url"
echo ""
